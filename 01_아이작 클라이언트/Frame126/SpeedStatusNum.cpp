#include "stdafx.h"
#include "SpeedStatusNum.h"
#include "ObjMgr.h"
#include "BmpMgr.h"
#include "KeyMgr.h"
#include "ScrollMgr.h"
#include "Player.h"
CSpeedStatusNum::CSpeedStatusNum()
{
}
...
void CSpeedStatusNum::Initialize(void)
{
	m_tInfo.fX = 60.f;		// 중점 X
	m_tInfo.fY = 275.f;		// 중점 Y
	...
	m_fSpeed = CObjMgr::Get_Instance()->Get_Player()->Get_Speed();
...
int CSpeedStatusNum::Update(void)
{
	m_iBomb = CObjMgr::Get_Instance()->Get_Player()->Get_Bomb();
	m_fSpeed = CObjMgr::Get_Instance()->Get_Player()->Get_Speed();
...
void CSpeedStatusNum::Late_Update(void)
{
	if (CKeyMgr::Get_Instance()->Key_Pressing('Q'))
		m_fAngle += 45.f;
}

void CSpeedStatusNum::Render(HDC hDC)
{
...
void CSpeedStatusNum::Release(void)
{

}

void CSpeedStatusNum::Motion_Change(void)
{
...
void CSpeedStatusNum::Insert_Texture(void)
{
	CBmpMgr::Get_Instance()->Insert_Bmp(L"../Image/UI/Number1.bmp", L"Number");

}

void CSpeedStatusNum::First_State(void)
{
...
void CSpeedStatusNum::CountAtk(void)
{
	float		fSpeedCount = m_fSpeed;
	...
	else if (fSpeedCount == 1.f)
	{
		m_pFrameKey = L"Number";
		m_eState = ONE;
	}
	else if (fSpeedCount == 2.f)
	{
		m_pFrameKey = L"Number";
		m_eState = TWO;
	}
	else if (fSpeedCount == 3.f)
	{
		m_pFrameKey = L"Number";
		m_eState = THREE;
	}
	else if (fSpeedCount == 4.f)
	{
		m_pFrameKey = L"Number";
		m_eState = FOUR;
	}
	else if (fSpeedCount == 5.f)
	{
		m_pFrameKey = L"Number";
		m_eState = FIVE;
	}
	else if (fSpeedCount == 6.f)
	{
		m_pFrameKey = L"Number";
		m_eState = SIX;
	}
	else if (fSpeedCount == 7.f)
	{
		m_pFrameKey = L"Number";
		m_eState = SEVEN;
	}
	else if (fSpeedCount == 8.f)
	{
		m_pFrameKey = L"Number";
		m_eState = EIGHT;
	}
	else if (fSpeedCount == 9.f)
	{
		m_pFrameKey = L"Number";
		m_eState = NINE;
	}

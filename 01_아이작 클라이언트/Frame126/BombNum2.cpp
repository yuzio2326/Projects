...
void CBombNum2::Initialize(void)
{
	m_tInfo.fX = 45.f;		// 중점 X
...
void CBombNum2::Late_Update(void)
{
	if (CKeyMgr::Get_Instance()->Key_Pressing('Q'))
		m_fAngle += 45.f;
}

void CBombNum2::Render(HDC hDC)
{
...
void CBombNum2::Release(void)
{

}

void CBombNum2::Motion_Change(void)
{
...
void CBombNum2::Insert_Texture(void)
{
	CBmpMgr::Get_Instance()->Insert_Bmp(L"../Image/UI/Number1.bmp", L"Number");

}

void CBombNum2::First_State(void)
{
...
void CBombNum2::CountBomb(void)
{
	int		iBombCount = m_iBomb/10;
	...
	else if (iBombCount == 1)
	{
		m_pFrameKey = L"Number";
		m_eState = ONE;
	}
	else if (iBombCount == 2)
	{
		m_pFrameKey = L"Number";
		m_eState = TWO;
	}
	else if (iBombCount == 3)
	{
		m_pFrameKey = L"Number";
		m_eState = THREE;
	}
	else if (iBombCount == 4)
	{
		m_pFrameKey = L"Number";
		m_eState = FOUR;
	}
	else if (iBombCount == 5)
	{
		m_pFrameKey = L"Number";
		m_eState = FIVE;
	}
	else if (iBombCount == 6)
	{
		m_pFrameKey = L"Number";
		m_eState = SIX;
	}
	else if (iBombCount == 7)
	{
		m_pFrameKey = L"Number";
		m_eState = SEVEN;
	}
	else if (iBombCount == 8)
	{
		m_pFrameKey = L"Number";
		m_eState = EIGHT;
	}
	else if (iBombCount == 9)
	{
		m_pFrameKey = L"Number";
		m_eState = NINE;
	}
